...
def nicinfo():
    ...
    for line in raw_data:
        ...
        if "HWaddr" in line:
            ...
            next_ip_line = True
            last_mac_addr = line
            # last_mac_addr = 'em1       Link encap:Ethernet  HWaddr F0:1F:AF:E0:6A:2C  '
...
def raminfo():
    ...
    for item in raw_ram_list:
        ...
        for i in item:
            # print(i)
            data = i.split(":")  # 对于分割之后长度为2的，是可以取到数据的项
            if len(data) == 2:
                key, v = data
...
def cpuinfo():
    ...
    cpu_model = raw_data["cpu_model"].split(":")  # ['model name\t', ' Intel(R) Xeon(R) CPU E5-2407 0 @ 2.20GHz']
    if len(cpu_model) > 1:
