...
def token_required(func):
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            token_md5_from_server = gen_token(username,timestamp,user_obj.userprofile.token)
            if token_md5_from_client != token_md5_from_server:
                response['errors'].append({"auth_failed":"Invalid username or token_id"})
            else:
                # 如果被劫持了，username,timestamp都不能改，改了就会token对不上，但如果不改时间，超时的话也不行
                # 如果时间也跟上了，只是替换了非法数据，那就得用redis，第一次来就存入，生存时间是5分钟，五分钟内相同的token又来的，视为非法
                if abs(int(time.time()) - int(timestamp)) > settings.TOKEN_TIMEOUT:
